...
writeOGR(roads,dsn="C:/Users/God/Documents/ML",layer="crap",driver="ESRI Shapefile")
...
Columns of the attribute table can be extracted just like in a `data.frame`.
```{r S4slots5}
cat$Attack
```
...
Most spatial operations are done using the package `rgeos`. One of the requirement of functions in rgeos is that objects **need to be projected**. When objects are really large, some operation may start to take a long time (e.g. intersections with `gIntersection`).
...
```{r temp, out.width="50%",eval=FALSE}
...
plot(canada,col=gray(0.6),border=gray(0.6),xlim=unname(bbox(x)[1,]),ylim=unname(bbox(x)[2,]))
plot(r,col=alpha(rev(heat.colors(100)),0.75),add=TRUE)
...
temp<-raster::getData('worldclim',var='tmean',res=0.5,lon=-71,lat=45)
temp
plot(temp)
...
Get data from RGBIF for Trillium erectum and measure the climatic enveloppe using different mean, min or max annual temperatures and measure the range of altitudes in which the species is found. Also, using a raster, compute the density of records in cells across the range of the species.

Here is a hint to download the data.
```{r ex2, eval=FALSE}
...
# extract values at the site of locations
e<-extract(x,r)
