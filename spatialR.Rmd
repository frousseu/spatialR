...
Most files for the workshop can be found here: [https://cdn.hebergix.com/fr/geospatialR/data/](https://cdn.hebergix.com/fr/geospatialR/data/)
...
monarchs <- occ_search(scientificName = "Danaus plexippus", hasCoordinate = TRUE, stateProvince = "Québec", country = "CA", limit = 5000)
...
<!--
```{r Ex2, include = FALSE, eval = FALSE, include = FALSE}
...
Calculate the proportion of forests in 10 random 2000m buffers across a landscape. Let's first generate a fictious landscape.

```{r ex0, tidy=FALSE, out.width="80%"}

set.seed(123)

n <- 20
x<-rnorm(n, -74, 0.1) # generate random points
y<-rnorm(n, 45, 0.1)


h <- data.frame(x,y) |> # turn it to spatial points 
     st_as_sf(coords = c("x", "y"), crs = 4326) |> 
     st_geometry() |>
     st_transform(32198) |>
     st_buffer(dist = 1000 * rgamma(n, 2, 1)) |> # build random gamma buffers around them 
...
```{r ex0sol, echo=TRUE, include=TRUE, tidy=FALSE, out.width = "60%"}
...
```{r ex4_1, out.width="80%", fig.height=5, fig.width=7}
...
```{r ex4_2, out.width="80%", fig.height=5, fig.width=7}

e <- elevation_30s(country = "CAN", path = getwd(), mask = TRUE)
q <- canada[canada$NAME_1 == "Québec", ]
e <- crop(e, q, mask = TRUE)
plot(e)
...
tavg <- worldclim_country("CAN", var = "tavg", path = getwd()) # this is too big and slow to download!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
tavg <- mean(tavg)

# download records
x <- occ_search(scientificName = "Trillium erectum", limit = 1000, hasCoordinate = TRUE, country = "CA")
x <- st_as_sf(x$data, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)
...
```{r tmap, out.width="100%", eval = FALSE, include = FALSE}
...
Create shapes with [mapedit](http://r-spatial.org/r/2017/06/09/mapedit_0-2-0.html) functionality.
