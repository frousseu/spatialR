...
# INTRODUCTION
The R spatial environment for manipulating spatial objects is mostly made of two suites of packages, one for shapefile vector data (sp, rgdal, rgeos) and one for raster/matrix data (raster). More recently the new package sf (for simple features) has been developped to simplify and improve manipulations and operations on spatial vector data. Here, I've chosen to focus on spatial objects from the sp packahe as a lot of other package cannot deal yet with sf objects. Also, once sp objects are understood, it should be really easy to us sf objects and functions instead.
