...
options(width=70)
knitr::opts_chunk$set(cache=TRUE,comment="##",eval=TRUE,error=TRUE,echo=TRUE,message=FALSE,warning=FALSE,tidy=FALSE,size="LARGE",fig.align="center",out.width="100%")
...
The R spatial environment for manipulating spatial objects is mostly made of two suites of packages, one for shapefile or vector data (sp, rgdal, rgeos) and one for raster/matrix data (raster). Vector data is mostly correspond to coordinates that make either points, lines or polygons, while raster data mostly correpsonds to regularly gridded data.

More recently the new package sf (simple features) has been developped to simplify and improve manipulations and operations on spatial vector data. Here, I've chosen to focus on spatial objects from the sp package as a lot of other package cannot deal with sf objects yet. Also, once sp objects are understood, it is really straigthforward to move to sf objects and functions instead.
...
roads<-readOGR(dsn="C:/Users/God/Documents/ML",layer="carreteras",verbose=FALSE,encoding="UTF-8")
...
cat<-read.csv("C:/Users/God/Documents/ML/LandEcoCorrected_addedvalue.csv",header=TRUE,stringsAsFactors=FALSE,sep=";",dec=",")
str(cat)
...
<!--Creating a shapefile of polygons is slightly more complex and requires a couple more steps. We can use pipes for the **magrittr** package to make this easier. To close a polygon, the last point of the matrix needs to be the first point that starts the polygon.-->
...
The function names and the `$` applied on a spatial object behave the same way as on a data.frame, while the `@` operator allows to extract objects stored in the slots of the object. Different methods are also available to extract different information fro spatial objects such bounding boxes (bbox), centroid coordinates (coordinates), etc. Type ?Spatial to learn more about them.

<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>

## Spatial operations

Most spatial operations are done using the package `rgeos`. One of the requirement of functions in rgeos is that objects should be projected. When objects are really large, some operation may start to take a long time (e.g. intersections with gIntersection).

**rgeos** requires object to be projected!
...
Load data with coordinates and measure distance between each locations.
...
r1<-raster::getData('alt',country='CAN')
r2<-raster::getData('alt',country='USA')[[1]]
...
r1<-raster::getData('worldclim',var='tmean',res=0.5,lon=-71,lat=45)
r2<-raster::getData('worldclim',var='tmin',res=0.5,lon=-71,lat=45)
r3<-raster::getData('worldclim',var='tmax',res=0.5,lon=-71,lat=45)
...
Generate a random set of points and store the density of points in a raster stack. Do something with NDVI data perhaps. Or download WorldClim data using getData function from raster and then compute mean annual temperature in Quebec using a region or a specific shapefile. Mean temperature and altit ude in the Bicknell range?
...
r<-raster::getData('worldclim',var='bio',res=2.5,lon=-70,lat=65)[[1]]
#bio2<-raster::getData('worldclim',var='bio',res=2.5,lon=-90,lat=65)[[1]]
#bio3<-raster::getData('worldclim',var='bio',res=2.5,lon=-110,lat=65)[[1]]
#alt<-raster::getData('alt',country='CAN')
#can<-getData('GADM', country='CAN', level=2)
#plot(r)
e<-extent(c(-74,-71,45,47))
r2<-crop(r,e)
plot(r2[[1]]/10)
...
r <- raster(nrows=200, ncols=200, xmn=0, xmx=100, ymn=0, 
            ymx=100)
