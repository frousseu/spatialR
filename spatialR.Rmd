...
<br><br>
You may see some code or package that still rely on the packages **sp**, **rgdal**, **rgeos** or **raster**, but these are now retired (for real!) and were entirely replaced by **sf** and **terra**.
<br><br>
...
plot(st_geometry(roads), col = gray(0.75), lwd = 4, axes = TRUE)
```
...
# As a shapefile
st_write(roads, dsn = "/home/frousseu/Documents/github/spatialR/data/roads.shp", append = FALSE)
...
```
The object returned is now an sf `data.frame`.
...
Build a spatial `data.frame` with 100 random locations centered on Sherbrooke, plot the results and write the results to a shapefile. Hint: use the function `runif` or `rnorm` to generate random values within a given range.
...
set.seed(1234)
n<-50
lon<-rnorm(n,-71.892, 0.04)
lat<-rnorm(n, 45.401, 0.04)
id<-1:n
...
st_write(x, dsn = "/home/frousseu/Documents/github/spatialR/sherbrooke.gpkg", append = FALSE)
...
```{r fig.width=6.5,fig.height=4,out.width="70%"}
...
<img style="height: 300px; align: center;" src="https://inaturalist-open-data.s3.amazonaws.com/photos/342006400/medium.jpg">
...
## Assign a CRS (*C*oordinate *R*eference *S*ystem)
...
See [https://epsg.io/](https://epsg.io/) to find CRS epsg's.
...
```{r proj_assign3, fig.width = 5, fig.height = 5, out.width = "60%"}


trille <- st_transform(trille, st_crs(quebec))
...
canada <- gadm('GADM', country='CAN', level=1) |> st_as_sf()
...
Most spatial operations are available in the package `sf`. One of the requirement for functions to work well for spatial operations in `sf` is that objects **are in a projected coordinate system**.
...
st_write(sherbrooke, "/home/frousseu/Documents/github/spatialR/sherbrooke.gpkg", append = FALSE)
...
sherbrooke <- st_read("/home/frousseu/Documents/github/spatialR/sherbrooke.gpkg")


#x <- st_sample(sherbrooke, 25, type = "random") |> st_as_sf()

plot(st_geometry(sherbrooke))
plot(st_geometry(x), add = TRUE)

mapview(sherbrooke)+mapview(x)

xin <- x[sherbrooke,]

s <- lengths(st_intersects(x, sherbrooke))
xout <- x[s == 0, ]

plot(st_geometry(sherbrooke))
plot(st_geometry(xin), pch = 16, col = "red", add = TRUE)
plot(st_geometry(xout), pch = 16, col = "blue", add = TRUE)
...
```{r polygons_op, fig.width=7, fig.height=6, out.width = "100%"}

# construct two buffers
x1 <- st_buffer(x[38, ], dist = 2000)
...
for(i in seq_along(op)){
  ...
  plot(st_geometry(get(op[i])(x1, x2)), add = TRUE, col = "forestgreen")
}
...
plot(st_geometry(roads), col = gray(0.7))
...
text(st_coordinates(st_centroid(buffs)), labels = round(dist/1000,1), col = "black", adj = c(-0.75,-1.5), xpd = TRUE, cex = 0.8)
```
...
r1 <- rast("/home/frousseu/Documents/github/spatialR/data/cdem_dem_021E.tif")
r2 <- rast("/home/frousseu/Documents/github/spatialR/data/cdem_dem_031H.tif")
...
r <- merge(r1, r2) # mosaic could also be used if rasters overlap
plot(r)
...
Let's extract elevation data for the Bicknell's Thrush occurrences we got previously.

```{r extract0}

plot(r)
trille <- st_transform(trille, st_crs(r))
trille$date <- format(as.Date(trille$eventDate),"%m-%d")
trille <- trille[trille$date >= "06-20" & trille$date <= "08-01", ]
plot(st_geometry(trille), add = TRUE)
...
buff <- st_buffer(trille, dist = 500)
elev <- crop(r, ext(-71.23, -70.7, 45.2, 45.5))
plot(elev)
...
system.time(extract(r, buff , fun = mean, exact = TRUE))
system.time(exact_extract(r, buff , fun = "mean"))
...
mn <- c(0, 100, 200, 400, 600, 800) # min
mx <- c(100, 200, 400, 600, 800, 1200) # max
...
rtrille <- rast(resolution = 50000, ext = ext(trille))
